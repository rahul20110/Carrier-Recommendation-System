...
if (isset($_POST['failure_button'])) {
    // If the "failure" button is clicked, set $success to false
    $success = false;
}
...
if ($success === true)
{
	...
	$stmt = $applicatF->runQuery("SELECT * FROM payments WHERE property_id=:email and razorpayOrderId=:razorpayOrderId and razorpayPaymentId <>'' ");
	$stmt->execute(array(":email" => $property_id, ":razorpayOrderId" => $razorpayOrderId));
	...
	if($applicatF->updatePayStatus($property_id,$razorpayOrderId, $razorpayPaymentId, $paymentStatus, $updatestamp ))
		{			
...
$updatestamp=date('Y-m-d h:i:s');
$applicatF->updatePayStatus($property_id,$razorpayOrderId, $razorpayPaymentId, $paymentStatus, $updatestamp );
$errMSG = "sorry , Query could no execute...";
}
...
<div class="container-contact100">
  <div class="wrap-contact100">
	...
	if (isset($errMSG)) {
		...
		<form id="errorForm" action="success.php" method="post">
			<input type="hidden" name="response" value="0">
			<input type="hidden" name="property_id" value="<?php echo $_SESSION['property_id']; ?>">
			<input type="hidden" name="payment_type" value="<?php echo $_SESSION['payment_type']; ?>">
			<input type="hidden" name="amount" value="<?php echo $_SESSION['toValue']; ?>">
		</form>
		<script>
		...
		<form id="successForm" action="success.php" method="post">
 ...
 <tr>
  ...
  <tr>
    ...
    <td class="txt-agl-lt"><?php echo $payment_type; ?></td>
  </tr>
  <tr>
