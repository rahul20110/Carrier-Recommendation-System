...
<section class="listings_01">
   ...
   <h1 class="heading">All Properties contracts signed are below </h1>
   ...
   <?php
      ...
      if($select->rowCount()> 0){
         ...
         <div class="box">
            ...
            <form action="contract2.php" method="POST" onsubmit="<?php if (is_null($seller_sign)) { ?>
            return confirm('The Contract has not been signed till now by the seller. We are working on it to make it possible as soon as possible. Do you want to see the signed contract?');
         <?php } else { ?>
            return true;
         <?php } ?>">
            <a href="view_property2.php?get_id=<?= $listing_id; ?>" class="btn">view property</a>
            <input type="hidden" name="pro_id" value="<?php echo $pro; ?>">
            <input type="submit" value="Save Contract" name="save" class="btn">
            
         </form>
         <?php
            if ($remaining != 0) {
            ?>
               <form action="pay2.php" method="POST" style="display: inline;">
                  <input type="hidden" name="pro_id" value="<?php echo $pro; ?>">
                  <input type="submit" value="Pay Remaining Amount <?php echo $remaining;?>" name="pay" class="btn">
               </form>
            <?php
            }
         ?>
