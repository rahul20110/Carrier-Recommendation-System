...
if(isset($_POST['post'])){
   ...
   $property_papers = $_FILES['property_papers']['name'];
   $property_papers = filter_var($property_papers, FILTER_SANITIZE_STRING);
   $property_papers_ext = pathinfo($property_papers, PATHINFO_EXTENSION);
   $rename_property_papers = create_unique_id() . '.' . $property_papers_ext;
   $property_papers_tmp_name = $_FILES['property_papers']['tmp_name'];
   $property_papers_size = $_FILES['property_papers']['size'];
   $property_papers_folder = 'property_proof/' . $rename_property_papers;

   if (!empty($property_papers)) {
      if ($property_papers_size > 2000000) {
         $warning_msg[] = 'Property papers file size is too large!';
      } else {
         move_uploaded_file($property_papers_tmp_name, $property_papers_folder);
      }
   } else {
      $rename_property_papers = '';
}
...
<section class="property-form">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="flex">
         ...
         <!-- added part for availability of dates for rent option -->
         <div class="box">
            <p>offer type <span>*</span></p>
            <select name="offer" required class="input" onchange="toggleAvailabilityFields(this.value)">
         ...
         <div id="availability-fields">
            <div class="box">
               <p>Available From <span>*</span></p>
               <input type="date" name="available_from" class="input" required>
            </div>
            <div class="box">
               <p>Available Till <span>*</span></p>
               <input type="date" name="available_till" class="input" required>
            </div>
         </div>

         <script>
         function toggleAvailabilityFields(offerType) {
            const availabilityFields = document.getElementById("availability-fields");
            
            if (offerType === "rent") {
               availabilityFields.style.display = "block"; // Show the fields
            } else {
               availabilityFields.style.display = "none"; // Hide the fields
            }
         }
         </script>
      ...
      <div class="flex"> 
         ...
         <!-- added part -->
         <div class="box">
            <p>Property Proof Papers (PDF) <span>*</span></p>
            <input type="file" name="property_papers" class="input" accept=".pdf" required>
         </div>   
