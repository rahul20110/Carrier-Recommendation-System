...
if(isset($_POST['h_search'])){
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE address LIKE '%{$h_location}%' AND type LIKE '%{$h_type}%' AND offer LIKE '%{$h_offer}%' AND price BETWEEN $h_min AND $h_max AND WHERE id NOT IN (SELECT property_id FROM employee_sign) ORDER BY date DESC");
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE address LIKE '%{$location}%' AND type LIKE '%{$type}%' AND offer LIKE '%{$offer}%' AND bhk LIKE '%{$bhk}%' AND status LIKE '%{$status}%' AND furnished LIKE '%{$furnished}%' AND price BETWEEN $min AND $max AND WHERE id NOT IN (SELECT property_id FROM employee_sign)ORDER BY date DESC");
   $select_properties->execute();

}else{
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE id NOT IN (SELECT property_id FROM employee_sign)ORDER BY date DESC LIMIT 6");
   $select_properties->execute();
}
